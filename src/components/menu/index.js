...
class Menu extends React.PureComponent {
    ...
    componentDidUpdate(prevProps) {
        if(JSON.stringify(prevProps.links) !== JSON.stringify(this.props.links)) {
            this.setAnchorsBounds();
            ...
            <nav className="menu">
                ...
                <ul className="menuContainer" ref={this.listContainer}>
                    {this.props.links.map(({ label, link }, index) => (
                        <li key={index}>
                            <a onClick={this.handleClick(index)} href={link}>{label}</a>
                        </li>
                    ))}
                    {showIndicator && <span className="menuIndicator" style={indicatorStyle}></span>}
                </ul>
            </nav>
        );
    }
